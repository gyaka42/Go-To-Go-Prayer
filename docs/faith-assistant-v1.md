...
- Backend counters live in the memory of one instance and expire at the end of their fixed minute or UTC-day window. The app also keeps a separate UTC-day question count on the device, migrated from that day's local answer history, so a Railway deployment or container restart cannot visibly restore the allowance on that installation. Clearing answer history does not clear this counter. Reinstalling the app or scaling Railway to multiple instances still requires a shared persistent rate-limit store for a server-enforced guarantee across resets and instances.
